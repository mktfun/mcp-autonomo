...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Generate-chat-response: User authenticated:", user.id, "Message:", message);
...
EXEMPLO 2 - Objetivo: "Olá, tudo bem?"
{
  ...
  "summary": "Este é um cumprimento social que não requer execução de ferramentas. Vou apenas responder educadamente."
    ...
    const plannerPrompt = `OBJETIVO DO USUÁRIO: "${message}"
...
Analise o objetivo e crie um plano de execução detalhado. Retorne APENAS o JSON do plano.`;
    ...
    // Create agent_action entry with status 'pending'
    ...
    if (actionError || !agentAction) {
      console.error("Failed to create agent action:", actionError);
      throw new Error("Failed to save plan");
    }

    console.log("Plan saved as agent action:", agentAction.id);
