...
serve(async (req) => {
  ...
  try {
    const { planLogId, projectId } = await req.json();

    if (!planLogId || !projectId) {
      throw new Error("planLogId and projectId are required");
    }
    ...
    console.log("Execute-plan: User authenticated:", user.id);

    // Fetch the plan from agent_logs
    const { data: planLog, error: planError } = await supabaseAdmin
      .from("agent_logs")
      .select("*")
      .eq("id", planLogId)
      .eq("user_id", user.id)
    ...
    const plan = planLog.tool_output.plan;
...
Seja conciso mas completo. Use markdown para formatação.`;
    ...
    console.error("Error in execute-plan:", error);
